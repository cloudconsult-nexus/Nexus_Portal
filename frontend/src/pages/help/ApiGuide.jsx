...
export const blocks = [
  ...
  para("Separate from the NCC on-call lookup above, and the opposite direction: this is the Portal calling OUT to NCC/Thrio to fetch/write Customer messages and customer records (services/ncc-client), not NCC calling in. Authenticates to NCC with a per-tenant username/password (Basic auth against login.thrio.com) rather than an API key — that's Thrio's only option today. These credential-management endpoints stay API-only, Global Admin only — the real end-user-facing feature is the Customer Messages page (frontend/src/pages/CustomerMessages.jsx), not this config surface. Secure Messaging's separate nav container was removed — Customer Messages is the one section for this now."),
  table(['Path', 'Notes'], [
  ...
  callout('Credential scoping: a Customer\'s own /ncc-config/:orgId override is tried first, falling back to the TAS-wide /ncc-config/tas/default if the Customer has none configured. Which of these actually matches how Nextiva provisions Thrio tenants per TAS/Customer is still open — Patrick (Nextiva) deferred that question back to Steve, since it\'s about how we provision Customers rather than something on Nextiva\'s side.', 'warning'),
  callout('Confirmed by Patrick (Nextiva) 2026-08-24, already reflected in code: the combined ?customerId=&acknowledged=false unacknowledged-messages filter works ("I tested each Postman request"); Create Customer\'s new id comes back in _id and customerId, not id (createCustomer\'s id-extraction was fixed to match); a username/password token is valid under 24h (a separate 6-month tenant-level token exists via an endpoint outside our current Postman collection); and there are no rate limits to worry about while testing. Still open: the real message/customer payload shapes (Patrick wants these captured from a live call rather than sent as samples), which needs Tom\'s NCC login working — the tenant admin account was repointed to tom@amiovox.ai but still needs a password reset before anything here has actually run against the live API.', 'warning'),
