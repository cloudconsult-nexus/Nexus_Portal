...
async function withResolvedBranding(settings) {
  if (!settings) return settings;
  return {
    ...settings,
    logo_url: await resolveAssetUrl(settings.logo_url),
    favicon_url: await resolveAssetUrl(settings.favicon_url),
  };
}
...
router.get('/public', async (req, res) => {
  ...
  res.json({ tasSettings: await withResolvedBranding(rows[0]) });
});
...
router.put('/', requireRole('global_admin'), async (req, res) => {
  ...
  if (updates.length === 0) return res.json({ tasSettings: await withResolvedBranding(existing) });
