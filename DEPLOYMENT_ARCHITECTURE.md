...
```
  ...
  Secret Manager ──▶ DB password, JWT secret, SMTP password, optional
                      integration secrets incl. NCC_API_KEY (mounted as env
                      vars, secrets.tf/iam.tf — see RUNBOOK.md's Secret
                      rotation section)
...
| 8 | NCC on-call lookup (Phase 5.4) | `backend/src/routes/onCall.js`, `middleware/serviceAuth.js` (`X-API-Key`, env var `NCC_API_KEY`, optional Secret Manager entry via `infra/secrets.tf`/`TF_VAR_ncc_api_key` — same conditional pattern as `smtp_password`), `lib/calendarService.js#getOnCallAt`. Inbound-only: NCC calls this app mid-call, this app never calls NCC — the reverse of every other external integration on this page. |
